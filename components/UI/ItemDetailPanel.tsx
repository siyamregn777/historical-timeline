...
const ItemDetailPanel: React.FC<Props> = ({ item, lang, onClose, onLearnMore }) => {
  ...
  return (
    <div className={`fixed sm:absolute z-[90] bottom-0 sm:bottom-6 ${isRTL ? 'left-0 sm:left-6' : 'right-0 sm:right-6'} w-full sm:max-w-md bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl border border-slate-100 p-8 animate-in slide-in-from-bottom duration-300 text-start`}>
      <button onClick={onClose} className={`absolute top-4 ${isRTL ? 'left-4' : 'right-4'} text-slate-400`}><i className="fa-solid fa-xmark"></i></button>
      <div className="flex flex-col gap-5">
        ...
        <p className="text-sm text-slate-600 leading-relaxed line-clamp-4">{item.description[lang]}</p>
        <footer className="pt-5 border-t border-slate-100 flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <span className="text-[9px] font-black text-slate-400">{item.id}</span>
          </div>
        </footer>
      </div>
    </div>
